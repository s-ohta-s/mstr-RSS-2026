# fit_varx.r
# VARX model estimation (0011 model) via SUR (systemfit):
#   y_i,t = Σ_j α_ij y_j,{t-1} + X_i β_i + ε_i,  ε ~ N(0, Σ) with Σ non-diagonal.
# No spatial terms (R = Λ = 0); A and Σ are non-diagonal.
...
fit_varx <- function(
  ...
  # Time-lag variables: lags of all y variables
  for (i in 1:k) {
    var_name <- y_vars[i]
  ...
  # Extract the A matrix (time-lag coefficients) from the coefficient names
  ...
  # Extract the regression coefficients (β)
  beta_list <- list()
  coef_tables <- list()
