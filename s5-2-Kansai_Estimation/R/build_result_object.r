...
# Unify the structure of the objects returned by each model's fit function
# (MSAR/MSEM/MGNS/diagonal/VARX/OLS) into class multivar_spatial.

# Helpers: extracting structure from the beta vector
...
#' @param fitted_vals reduced-form (trend, d=0) fitted value vector ŷ
#'   (auto-computed from beta/X/W if NULL; independent models must pass it
#'   because their data_list has no X)
...
build_result_object <- function(
  ...
  # Fitted values ŷ (reduced-form trend prediction, d = 0), identical to pred
  # in the fitted_residuals CSVs:
  #   MSAR/MGNS (incl. Ind variants): ŷ = (I − R̂⊗W)⁻¹ Xβ̂
  #   MSEM/IndSEM/VARX/OLS          : ŷ = Xβ̂
  # The time-lag term A·y(t−1) is part of Xβ̂ (lag columns included in X).
  # Note: ŷ ≠ y − ε̂ for spatial models (ε̂ stays in residuals$raw).
  if (is.null(fitted_vals)) {
    fitted_vals <- tryCatch({
      if (model_type %in% c("VARX", "IndReg")) {
        if (!is.null(residuals_raw) && !is.null(data_list$y)) {
          as.numeric(data_list$y) - as.numeric(residuals_raw)
        } else NULL
      } else if (!is.null(beta) && !is.null(data_list$X)) {
        compute_fitted_multivar(model_type, R, beta,
                                data_list$X, data_list$W, k, n)
      } else NULL
    }, error = function(e) NULL)
  }

  # Pseudo R²: R²_pseudo,k = corr(y_k, ŷ_k)² with the trend prediction ŷ above
  R2 <- NULL
  R2_adj <- NULL
  R2_individual <- NULL
  if (!is.null(fitted_vals) && !is.null(data_list$y)) {
    r2_result <- tryCatch({
    ...
    if (!is.null(r2_result)) {
      ...
      R2_individual <- r2_result$R2_pseudo_individual
      names(R2_individual) <- data_list$data_info$y_vars
      R2_adj <- NA  # no adjusted version for pseudo R²
    }
  }

  # Adjusted R²: only for equation-by-equation (independent) models, as the
  # mean of the per-equation adjusted R²; not definable for full models (NA).
  if (!is.null(individual_models)) {
    adj_vals <- unlist(lapply(individual_models, function(im) {
      if (!is.null(im$fit) && !is.null(im$fit$R2_adj)) im$fit$R2_adj else NA_real_
    }))
    if (length(adj_vals) > 0 && any(!is.na(adj_vals))) {
      R2_adj <- mean(adj_vals, na.rm = TRUE)
    }
  }
  ...
  result <- structure(
    list(
      ...
      # Residuals (raw = innovation residual ε̂ of the model's likelihood)
      residuals = list(
      ...
      # Fitted values (trend = reduced-form d=0 prediction ŷ; ŷ ≠ y − ε̂ for spatial models)
      fitted = list(
        trend = if (!is.null(fitted_vals)) as.numeric(fitted_vals) else NULL
      ),
      ...
      # inference/vcov/hessian are added later by add_inference
      inference = NULL,
      vcov      = NULL,
      hessian   = NULL,
...
# Minimal data_list construction helper for diagonal models
