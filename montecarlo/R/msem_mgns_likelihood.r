...
# Core likelihood and parameter-update functions for the MSEM/MGNS models
...
compute_LambdaW_times_v <- function(Lambda_mat, W, v, k, n, verbose = FALSE) {
  # Reshape v into an n×k matrix, apply W once to all columns, then combine
  # blocks: block_i = Σ_j λij·(W v_j) <=> (W·V) %*% t(Λ), stacked by column.
  # Works for both dense and Matrix::sparse W.
  v_matrix <- matrix(v, nrow = n, ncol = k)
  WV <- as.matrix(W %*% v_matrix)
  result <- as.numeric(WV %*% t(Lambda_mat))
...
# 2. MSEM (Spatial Error Model) implementation
...
compute_log_likelihood_msem <- function(Lambda_mat, beta, Sigma, y, X, W, eigen_W, k, n, verbose = 0) {
  ...
  # Compute the residuals z = (I - Λ⊗W)(y - Xβ)
...
update_beta_msem <- function(Lambda_mat, Sigma, y, X, W, k, n, ridge_eps = 1e-6, verbose = 0,
  ...
  # X'(I-Λ'⊗W')(Σ^{-1}⊗I)(I-Λ⊗W)X and X'(I-Λ'⊗W')(Σ^{-1}⊗I)(I-Λ⊗W)y.
  # (I-Λ⊗W)X / (I-Λ⊗W)y and their cross blocks depend only on Λ, not Σ, so
  # callers iterating over Σ pass them precomputed via gls_blocks.
  if (is.null(gls_blocks)) {
    ILambdaWy <- compute_I_minus_LambdaW_times_v(Lambda_mat, W, y, k, n)
    ILambdaWX <- X - compute_MW_times_M(Lambda_mat, W, X, k, n)
    gls_blocks <- precompute_gls_blocks(ILambdaWX, ILambdaWy, k, n)
  }

  gls <- assemble_gls_from_blocks(gls_blocks, Sigma_inv, k)
  XtAX <- gls$XtAX
  XtAy <- gls$XtAy
...
update_Sigma_msem <- function(Lambda_mat, beta, y, X, W, k, n, ridge_eps = 1e-6, verbose = 0) {
  ...
  # Compute the elements of Σ (block structure)
  Sigma_new <- matrix(0, k, k)

  for (i in 1:k) {
    for (j in i:k) {  # compute only j >= i (using symmetry)
      zi_idx <- ((i-1)*n + 1):(i*n)
      zj_idx <- ((j-1)*n + 1):(j*n)
      ...
      if (i != j) {
        Sigma_new[j,i] <- Sigma_new[i,j]  # ensure symmetry
      }
    }
  }
...
iterate_beta_sigma_msem <- function(
  ...
  # Λ is fixed inside this loop: precompute the Λ-dependent GLS blocks once
  ILambdaWy_pre <- compute_I_minus_LambdaW_times_v(Lambda_mat, W, y, k, n)
  ILambdaWX_pre <- X - compute_MW_times_M(Lambda_mat, W, X, k, n)
  gls_blocks <- precompute_gls_blocks(ILambdaWX_pre, ILambdaWy_pre, k, n)
...
# 3. MGNS (General Nesting Spatial Model) implementation
...
compute_log_likelihood_mgns <- function(R, Lambda_mat, beta, Sigma, y, X, W, eigen_W, k, n, verbose = 0) {
  ...
  # Compute the residuals z = (I - Λ⊗W)(y - (R⊗W)y - Xβ)
...
update_beta_mgns <- function(R, Lambda_mat, Sigma, y, X, W, k, n, ridge_eps = 1e-6, verbose = 0,
  ...
  # (I-Λ⊗W)X, (I-Λ⊗W)(I-R⊗W)y and their cross blocks depend only on (R, Λ),
  # not Σ, so callers iterating over Σ pass them precomputed via gls_blocks.
  if (is.null(gls_blocks)) {
    RWy <- compute_RW_times_y(R, W, y, k, n, verbose = FALSE)
    IRWy <- y - RWy
    ILambdaW_IRWy <- compute_I_minus_LambdaW_times_v(Lambda_mat, W, IRWy, k, n)
    ILambdaWX <- X - compute_MW_times_M(Lambda_mat, W, X, k, n)
    gls_blocks <- precompute_gls_blocks(ILambdaWX, ILambdaW_IRWy, k, n)
  }
...
iterate_beta_sigma_mgns <- function(
  ...
  # (R, Λ) are fixed inside this loop: precompute the GLS blocks once
  RWy_pre <- compute_RW_times_y(R, W, y, k, n, verbose = FALSE)
  ILambdaW_IRWy_pre <- compute_I_minus_LambdaW_times_v(Lambda_mat, W, y - RWy_pre, k, n)
  ILambdaWX_pre <- X - compute_MW_times_M(Lambda_mat, W, X, k, n)
  gls_blocks <- precompute_gls_blocks(ILambdaWX_pre, ILambdaW_IRWy_pre, k, n)
