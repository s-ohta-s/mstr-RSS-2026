# model_methods.r
# S3 methods of the multivar_spatial class (print/summary/coef/vcov/fitted/
# residuals/logLik/AIC/BIC/confint), model comparison, and significance tests
# (add_inference).
...
fitted.multivar_spatial <- function(object, ...) {
  ...
  # Reduced-form trend prediction ŷ (d = 0), computed in build_result_object
  if (!is.null(object$fitted$trend)) {
    return(object$fitted$trend)
  }

  warning("Cannot extract fitted values (fitted$trend is missing)")
  return(NULL)
}
...
# Significance-test computation (run after estimation)
...
create_coefficient_table <- function(object, se_beta, se_R, se_Lambda) {
  ...
  # Spatial parameters R (MSAR, MGNS)
  ...
  # Spatial parameters Λ (MSEM, MGNS)
