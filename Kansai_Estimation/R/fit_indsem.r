# fit_indsem.r
# Diagonal-constrained multivariate MSEM estimation (0ddd model):
# each variable estimated separately with spatialreg::errorsarlm(),
# so Λ, A, Σ are diagonal and R = 0.

#' Diagonal-constrained multivariate MSEM estimation (0ddd model)
#' 
#' Estimate each variable separately with errorsarlm() and merge the results
#' 
...
fit_indsem <- function(
  ...
  if (verbose) {
    ...
    cat("=== Diagonal-constrained MSEM estimation (0ddd model) ===\n")
  ...
  # S1-S2: estimate each variable separately with errorsarlm()
  if (verbose) {
    cat("\n")
    cat(paste(rep("-", 70), collapse=""), "\n")
    cat("Per-variable MSEM estimation (errorsarlm)\n")
    cat(paste(rep("-", 70), collapse=""), "\n")
  }
  ...
  for (i in 1:k) {
    ...
    tryCatch({
      ...
      # Residuals (innovation ε̂ = (I − λ̂W)(y − Xβ̂)) and
      # fitted values (trend ŷ = Xβ̂, d = 0)
      ...
      # Likelihood-ratio test (spatial error vs OLS)
      ...
      # R² is based on the trend prediction ŷ = Xβ̂; residual = y − ŷ, not the innovation ε̂
      y_i <- df_i$y
...
#' print method
print.multivar_indsem <- function(x, digits = 4, ...) {
  
  cat("\n")
  cat(paste(rep("=", 60), collapse=""), "\n")
  cat("Diagonal-constrained MSEM (0ddd model)\n")
  cat(paste(rep("=", 60), collapse=""), "\n")
...
#' summary method
summary.multivar_indsem <- function(object, digits = 4, ...) {
  
  x <- object
  
  cat("\n")
  cat(paste(rep("=", 70), collapse=""), "\n")
  cat("Diagonal-constrained MSEM - detailed results (0ddd model)\n")
  cat(paste(rep("=", 70), collapse=""), "\n")
  ...
  for (i in 1:x$data_info$k) {
    ...
    if (!is.na(sp$LR_statistic)) {
      LR_result <- ifelse(sp$LR_p_value < 0.05, "reject λ=0", "fail to reject λ=0")
...
#' coef method
coef.multivar_indsem <- function(object, type = "all", ...) {
...
#' logLik method
logLik.multivar_indsem <- function(object, ...) {
...
#' AIC method
AIC.multivar_indsem <- function(object, ..., k = 2) {
  return(object$fit$AIC)
}


#' BIC method
BIC.multivar_indsem <- function(object, ...) {
  return(object$fit$BIC)
}


#' residuals method
residuals.multivar_indsem <- function(object, type = "raw", ...) {
...
#' fitted method (trend prediction ŷ = Xβ̂, d = 0)
fitted.multivar_indsem <- function(object, ...) {
