...
#'
#' @return list(y, X, W, W_listw, y_lag, eigen_W, n, k, regions, p0, data_info)
...
build_design_matrix_extended <- function(
  ...
  # Build the lag variable part (only when a time lag is included)
...
compute_vcov_beta <- function(X, Sigma, k, n, Lambda_mat = NULL, W = NULL) {
  ...
  if (!is.null(Lambda_mat) && !is.null(W)) {
    ...
    for (col in 1:p) {
      AX[, col] <- compute_I_minus_LambdaW_times_v(Lambda_mat, W, X[, col], k, n)
    }
  } else {
    AX <- X
  }
...
if (gamma > 0) {
    # γ>0: sandwich estimator. Since this is the Hessian of the spatial
    # parameters only, D = I, so H_p = H + γI, Avar = H_p⁻¹ H H_p⁻¹
    k1 <- nrow(hessian)
...
compute_r_squared <- function(y, fitted, residuals, loglik = NULL, num_params = NULL) {
  ...
  # 4. Pseudo R² (McFadden), using the log-likelihood of the null
...
# 4-1b. Fitted values ŷ of the multivariate model (trend prediction, d = 0)

#' Compute the reduced-form (trend) fitted values ŷ of the multivariate model
#'
#' Official definition of ŷ (shrinkage function φ_d with d = 0):
#'   MSAR/MGNS (R≠0): ŷ = S_R⁻¹ Xβ = (I - R⊗W)⁻¹ Xβ
#'   MSEM/VARX/OLS (R=0): ŷ = Xβ
#' The time-lag term A·y(t−1) is part of Xβ (the lag columns are included in X).
#' Used for the pseudo R² (build_result_object.r) and the pred column of the
#' fitted_residuals CSVs (inference_and_export.r).
#' Note: ŷ ≠ y − ε̂ for spatial models; ε̂ (the innovation residual) is kept
#' separately in result$residuals$raw.
#'
...
#' Reduced-form (trend, d = 0) fitted values of a single-equation sarlm model
#'
#' For the per-equation models of the independent (diagonal) fits:
#'   lagsarlm / sacsarlm (spatial lag present): ŷ = (I_n − ρ̂W)⁻¹ Xβ̂
#'   errorsarlm (spatial error only)          : ŷ = Xβ̂
#' Equivalent to predict(model, pred.type = "TS") for lag models, but computed
#' directly from the definitional formula for robustness.
#'
#' @param model sarlm object (lagsarlm / errorsarlm / sacsarlm)
#' @param W n×n spatial weight matrix (dense)
#' @return n×1 fitted value vector
compute_fitted_sarlm_trend <- function(model, W) {
  trend <- as.numeric(model$X %*% model$coefficients)
  rho <- model$rho
  # errorsarlm: no spatial lag (rho is NULL) -> trend only
  if (is.null(rho) || length(rho) == 0 || !is.finite(rho)) return(trend)
  as.numeric(solve(diag(nrow(W)) - rho * W, trend))
}
...
#'
#' ŷ is the reduced-form (trend, d = 0) prediction computed by
#' compute_fitted_multivar() / compute_fitted_sarlm_trend()
#' (built in build_result_object.r), identical to pred in the
#' fitted_residuals CSVs.
#'
#' @param y Kn×1 observed value vector
#' @param fitted Kn×1 fitted value vector (trend prediction ŷ)
#' @param k number of variables
#' @param n number of regions
