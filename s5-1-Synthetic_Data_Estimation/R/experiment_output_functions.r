...
# Table-generation functions for numerical experiment Phase 4
...
extract_params_uniform <- function(result, model_id, gamma = 0,
  ...
  se_Lambda_prof <- result$std_errors$Lambda
  
  # Full model: recompute Ψ-based SE (if not set)
  ...
  # (a) β regression coefficients (intercept + regressors)
  ...
  if (!is.null(beta_vec)) {
    ...
    if (include_time_lag && idx <= length(beta_vec)) {
      ...
      # No time lag, or VARX (A is stored separately)
    ...
    # beta0 is a list of named numeric vectors. It may include time-lag terms (e.g., "y1_lag").
    # Time-lag terms are output separately as the A matrix, so they are excluded here.
    ...
    for (i in 1:k) {
      ...
      if (is.null(coef_i)) {
        # If empty: output NA for the intercept + regressors
    ...
    # Obtain the A matrix: VARX -> coefficients$A/$alpha, diagonal models -> lag terms in beta0
    A_diag <- matrix(NA, k, k)
    A_se   <- matrix(NA, k, k)
    
    # (1) Prefer coefficients$A or $alpha if present (for VARX)
    ...
    if (!is.null(A_from_result)) {
      ...
      # (2) Diagonal models: extract diagonal elements from lag terms in beta0
      # OLS has no time lag
      for (i in 1:k) {
        vn <- y_vars[i]
        coef_i <- beta0[[vn]]
  ...
  for (i in 1:k) for (j in 1:k) {
    ...
    se_p <- if (!is.null(se_Lambda_prof)) se_Lambda_prof[i, j] else NA
  ...
  # pseudo_R2 / adj_R2: computed from individual_models if result$fit$R2 is NULL
...
build_model_selection_table <- function(
  ...
  for (mid in model_ids) {
    ...
    # If a pBIC-criterion result exists, get the loglik for BIC
    r_bic <- if (!is.null(results_pBIC[[mid]])) results_pBIC[[mid]] else r_aic
    
    # R2: computed from individual_models if result$fit$R2 is NULL
    r2_val <- r_aic$fit$R2
...
# 4. Comparison table against true values (bias evaluation)
...
build_se_comparison_table <- function(
  ...
  for (mid in names(results_list)) {
    ...
    # Exclude goodness-of-fit rows (not subject to SE comparison)
    ...
    # Compute SE_ratio (β coefficients only)
    df$SE_ratio <- ifelse(
...
# 6. CSV export of the γ-search trajectory table
