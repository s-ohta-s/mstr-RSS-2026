# msar_likelihood.r
# Core likelihood and parameter-update functions for the MSAR model (Spatial Lag of Y)

# 1. Likelihood-related computations
...
compute_log_likelihood <- function(R, beta, Sigma, y, X, W, eigen_W, k, n, verbose = 0) {
  ...
  # Compute the residuals z = (I - R⊗W)y - Xβ
  RWy <- compute_RW_times_y(R, W, y, k, n, verbose = FALSE)
  z <- y - RWy - X %*% beta

  # Compute Q = z'(Σ^{-1}⊗I)z
  # This can be computed block by block
...
update_beta <- function(R, Sigma, y, X, W, k, n, ridge_eps = 1e-6, verbose = 0,
  ...
  # X'(Σ^{-1}⊗I)X and X'(Σ^{-1}⊗I)(I-R⊗W)y. The cross blocks depend only on
  # (X, R), not Σ, so callers iterating over Σ pass them precomputed.
  if (is.null(gls_blocks)) {
    RWy <- compute_RW_times_y(R, W, y, k, n, verbose = FALSE)
    y_transformed <- y - RWy
    gls_blocks <- precompute_gls_blocks(X, y_transformed, k, n)
  }

  gls <- assemble_gls_from_blocks(gls_blocks, Sigma_inv, k)
  XtSigmaInvX <- gls$XtAX
  XtSigmaInvy <- gls$XtAy
...
iterate_beta_sigma <- function(
  ...
  # R is fixed inside this loop: precompute the R-dependent GLS blocks once
  RWy_pre <- compute_RW_times_y(R, W, y, k, n, verbose = FALSE)
  gls_blocks <- precompute_gls_blocks(X, y - RWy_pre, k, n)
