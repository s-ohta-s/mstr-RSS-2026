...
# Functions to export estimation results to CSV
# Output formats:
#   (1) Parameter estimates: parameter, estimate, std_error, z_value, p_value, signif
#       as a long-format table. Goodness-of-fit metrics are appended to the same table.
#       → export_results_csv(), export_multiple_models_csv()
#   (2) Fitted values / residuals: same long format as the input data (region, time, <var>_obs/_pred/_resid)
#       → export_fitted_residuals_csv(), export_fitted_residuals_all_models()

# 1. Sandwich variance computation
...
compute_sandwich_vcov <- function(H, gamma) {
  ...
  # Check that diagonal elements are not negative
  diag_vcov <- diag(vcov)
  if (any(diag_vcov < 0)) {
...
# 2. Standard-error computation for Σ
...
export_results_csv <- function(
  ...
  # For goodness-of-fit metrics (no SE/z/p)
  ...
  if (include_time_lag && !is.null(result$coefficients$alpha)) {
    ...
    if (!is.null(se_beta) && !is.null(result$coefficients$beta)) {
      # Compute the total count of β0 to locate the indices of the α part
      n_beta0 <- 0
      for (i in 1:k) {
  ...
  # Time-lag coefficients (the tail of the β vector is already output as the A matrix)
  # -> skip the β indices
  if (include_time_lag) {
  ...
  if (!is.null(result$coefficients$beta)) {
    fitted_vals <- tryCatch({
      ...
      warning("Error computing ŷ: ", e$message)
      NULL
    })
    ...
    if (!is.null(fitted_vals)) {
      r2_result <- tryCatch({
        ...
        warning("Error computing R²: ", e$message)
        NULL
      })
    }
...
# 4. Batch export of multiple models
...
# 5. Export fitted values / residuals (same long format as the input data)

#' Export fitted values / residuals to CSV in the same long format as the input data
#'
#' Output columns: region, time, <yvar>_obs, <yvar>_pred, <yvar>_resid (per dependent variable)
#' Same row structure as the slice of the input data (region, time, y1, y2, ...) at time = the used time point.
#'
#' The fitted values (pred) and residuals (resid) satisfy:
#'   obs = pred + resid
#' Since the residuals use the model-specific residuals (result$residuals$raw),
#' they are consistent with the residuals used in the corresponding likelihood / Σ estimation:
#'   - OLS/VARX: ε = y − Xβ          → pred = Xβ
#'   - MSAR/MGNS: ε = (I−R⊗W)y − Xβ   -> pred = obs − ε (fitted value using the observed neighborhood)
#'   - MSEM     : ε = (I−Λ⊗W)(y−Xβ)  → pred = obs − ε
#'
#' @param result object returned by build_result_object()
#' @param output_file output CSV file path
#' @param verbose verbose output
...
export_fitted_residuals_csv <- function(
  ...
  # Do not export when observed values or residuals are missing
  ...
  if (length(obs) != k * n || length(resid) != k * n) {
    warning(sprintf("The length of observed values/residuals does not match kn=%d (obs=%d, resid=%d). Skipping.",
                    k * n, length(obs), length(resid)))
    return(invisible(NULL))
  }

  # Fitted = observed − residual (exactly satisfies obs = pred + resid)
  ...
  # Time point used (the model is estimated at a single time point)
