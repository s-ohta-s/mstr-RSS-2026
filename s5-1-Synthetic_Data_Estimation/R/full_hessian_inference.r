...
# Significance tests for β based on the joint Hessian of all parameters
# Overview:
#   The current compute_vcov_beta() treats R, Λ as known (fixed at their estimates)
#   and computes the conditional variance Ψ of β.
#
#   This file numerically computes the joint Hessian over all parameters
#   θ = (β', vec(R)', vec(Λ)')' and, via a sandwich estimator,
#   obtains a variance that includes the effect of R, Λ estimation uncertainty propagating to β.
# Theory:
...
# Usage:
#   source("full_hessian_inference.r")
#   result <- add_full_inference(result, gamma = 5)

# 1. Log-likelihood with Σ profiled out (a function of θ = (β, spatial))
...
# 2. Joint Hessian computation over all parameters
...
# 3. Sandwich variance estimation — with the D matrix
...
# 4. Construction of Z statistics
...
add_full_inference <- function(object, gamma = NULL, verbose = FALSE) {
  ...
  # Step 5: build a comparison table against the existing Ψ-based SE
  if (verbose) {
...
# 6. CSV-output helper: CSV including the full-Hessian results
